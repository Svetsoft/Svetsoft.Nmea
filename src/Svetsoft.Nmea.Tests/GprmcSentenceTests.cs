...
namespace Svetsoft.Geography.Positioning.Tests
{
    ...
    public class GprmcSentenceTests
    {
        ...
        {
            var gprmcSentence = NmeaSentence.ParseGprmc("$GPRMC,081836,A,3751.65,S,14507.36,E,000.0,360.0,130998,011.3,E*62");
        ...
        {
            var result = NmeaSentence.TryParseGprmc("$GPRMC,081836,A,3751.65,S,14507.36,E,000.0,360.0,130998,011.3,E*6", out GprmcSentence gprmcSentence);

            Assert.AreEqual(true, result);

            // NmeaSentence (Inherited)
            Assert.AreEqual("$GPRMC,081836,A,3751.65,S,14507.36,E,000.0,360.0,130998,011.3,E*6", gprmcSentence.Sentence);
            Assert.AreEqual("GPRMC", gprmcSentence.MessageType);
            Assert.AreEqual("62", gprmcSentence.Checksum);
            CollectionAssert.AreEqual(new[]
            {
